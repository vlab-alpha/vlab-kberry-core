...
import tools.vlab.kberry.core.knx.devices.LuxCategory;
...
public class LuxSensor extends KNXDevice {
    ...
    private LuxSensor(PositionPath positionPath, Integer refreshData) {
        super(positionPath, refreshData, KnxCommand.LUX_VALUE_ACTUAL);
    ...
    public void load() throws BAOSReadException {
        if (this.currentLux.isOlderThan(1000 * 60)) {
            this.get(KnxCommand.LUX_VALUE_ACTUAL).flatMap(DataPoint::getFloat9).ifPresent(lux -> {
                currentLux.set(lux);
                smoothedLux.set(lux);
            });
        }
