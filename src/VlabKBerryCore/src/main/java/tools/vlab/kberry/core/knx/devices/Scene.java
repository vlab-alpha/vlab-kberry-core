...
import static tools.vlab.kberry.core.knx.devices.KnxCommand.SCENE_ACTIVATION;
...
public class Scene extends KNXDevice {
    ...
    public void activateScene(int sceneNumber) {
        if (sceneNumber >= 1 && sceneNumber <= 64) {
            this.set(KnxCommand.SCENE_ACTIVATION, sceneNumber);
        } else {
    ...
    protected void received(KnxCommand command, DataPoint dataPoint) {
        if (command == KnxCommand.SCENE_ACTIVATION) {
