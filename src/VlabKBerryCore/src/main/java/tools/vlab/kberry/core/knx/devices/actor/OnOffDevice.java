...
public class OnOffDevice extends KNXDevice {
    ...
    protected OnOffDevice(PositionPath positionPath, Integer refreshData, String type) {
        super(positionPath, refreshData, KnxCommand.ON_OFF_SWITCH, KnxCommand.ON_OFF_STATUS);
    ...
    public void on() {
        if (!currentValue.get()) {
            this.set(KnxCommand.ON_OFF_SWITCH, true, true);
        }
    }
    ...
    public void load() throws BAOSReadException {
        this.get(KnxCommand.ON_OFF_SWITCH).flatMap(DataPoint::getBoolean).ifPresent(this.currentValue::set);
    }
