...
public class Led extends KNXDevice {
    ...
    private Led(PositionPath positionPath, Integer refreshData) {
        super(positionPath,
                refreshData,
                KnxCommand.RGB_COLOR_CONTROL,
                KnxCommand.RGB_COLOR_STATUS,
    ...
    public void setRGB(RGB rgb) {
        if (rgb.isBlack()) {
            ...
            this.set(KnxCommand.RGB_COLOR_CONTROL, rgb);
        }
    }
    ...
    public void load() throws BAOSReadException {
        this.get(KnxCommand.RGB_COLOR_STATUS).flatMap(DataPoint::getRGB).ifPresent(color::set);
        this.get(KnxCommand.ON_OFF_SWITCH).flatMap(DataPoint::getBoolean).ifPresent(status::set);
    }
}
