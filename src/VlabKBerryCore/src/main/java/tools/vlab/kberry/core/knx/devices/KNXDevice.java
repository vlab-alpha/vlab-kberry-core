...
import tools.vlab.kberry.core.knx.baos.*;
...
public abstract class KNXDevice {
    ...
    private final KnxCommand[] cmd;
    private final ConcurrentHashMap<KnxCommand, BAOSObject> BAOMap = new ConcurrentHashMap<>();
    private final Integer refreshIntervallMs;
    @Getter
    private final String id = UUID.randomUUID().toString();
    private final ExecutorService executor = Executors.newSingleThreadExecutor();

    protected KNXDevice(PositionPath positionPath, Integer refreshIntervallMs, KnxCommand... cmd) {
        this.positionPath = positionPath;
        this.cmd = cmd;
        this.refreshIntervallMs = refreshIntervallMs;
    }

    protected List<KnxCommand> getCommands() {
    ...
    protected abstract void received(KnxCommand command, DataPoint dataPoint);

    private void validate(KnxCommand command) {
    ...
    protected void set(KnxCommand command, boolean value) {
        set(command, value, false);
    }

    protected void set(KnxCommand command, boolean value, boolean priority) {
    ...
    protected void set(KnxCommand command, HeaterMode mode) {
        this.validate(command);
        var bao = BAOMap.get(command);
    ...
    protected void set(KnxCommand command, int value) {
        set(command, value, false);
    }

    protected void set(KnxCommand command, int value, boolean priority) {
        this.validate(command);
        var bao = BAOMap.get(command);
    ...
    protected void set(KnxCommand command, float value) {
        set(command, value, false);
    }

    protected void set(KnxCommand command, float value, boolean priority) {
        this.validate(command);
        var bao = BAOMap.get(command);
        switch (bao.datapointType()) {
    ...
    protected void set(KnxCommand command, RGB value) {
        set(command, value, false);
    }

    protected void set(KnxCommand command, RGB value, boolean priority) {
        this.validate(command);
        var bao = BAOMap.get(command);
    ...
    public Optional<DataPoint> get(KnxCommand command) throws BAOSReadException {
        var bao = BAOMap.get(command);
